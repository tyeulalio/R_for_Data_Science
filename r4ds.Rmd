...
4. Compare and contrast `coord_cartesian()` vs `xlim()` or `ylim()` when zooming in on a histogram. What happens if you leave binwidth unset? What happens if you try and zoom so noly half a bar shows?
...
2. What variable in the diamonds dataset is the most important for predicting the price of a diamond? How is that variable correlated with cut? Why does the combination of those two relationships lead to lower quality diamonds being more expensive?
...
5. Two dimensional plots reveal outliers that are not visible in one dimonsional plots. For example, some points in the plot below have an unusual combination of x and y values, which makes the points outliers even though their x and y values appear normal when examined separately.
```{r}
ggplot(data = diamonds) +
...
Not a lot of coding here so skipping these sections.

# Chapter 8: Workflow Projects
<a id="top"></a>

Just some reading so skipping this chapter.

# Chapter 9: Introduction
<a id="top"></a>

Introduction to the new Wrangle section of the book.
...
If you're already familiary with `data.frame()`, note that `tibble()` does much less:
...
- `options(tibble.print_max = n, tibble.print_min = m)`: if more than n rows, print only m rows. Use `options(tibble.print_min = Inf`) to alays show all rows.
...
```{r}
str(df[,c('abc')])
str(df[,c('abc'), drop=FALSE])
```


3. If you have the name of a variable stored in an object, e.g. `var <- "mpg"`, how can you extract the reference variables from a tibble?
```{r}
var <- "mpg"

mtcars_tibble <- as.tibble(mtcars)

# use var in square brackets
mtcars_tibble[[var]]
...
# 4. Renaming the columns to one, two, three
annoying <- annoying %>%
  rename(one=`1`,
         two=`2`,
         three=`3`)
...
Here we'll learn how to load flat files in R with the `readr` package, which is part of the core tidyverse.

Most of readr's functions are concered with turning flat files into data frames:
...
In both cases, `read_csv()` uses the first line of the data for column names, which is a very common convention. There are two cases where you might want to tweak this behavior:
...
This is all you need to know to read ~75% of CSV files that you'll encounter in practice. You can easily adapt what you've learned to read tab separated files with `read_tsv()` and fixed width files with `read_fwf()`.  
